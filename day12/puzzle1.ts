...
class CaveMap {
  ...
  findPaths() {
    return this.findPathsRecursive('start', []);
  }

  private findPathsRecursive(from: string, usedSmallCaves: string[]): number {
    ...
    return caves.reduce((acc, curr) => {
      return acc + this.findPathsRecursive(curr, visitedSmallCaves);
    }, 0);
  }
}
